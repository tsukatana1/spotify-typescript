...
export class SpotifyRequester {
    ...
    static async getOauthToken(clientID: string, clientSecret: string): Promise<TokenEndpoint> {
        const fetchReq = await fetch(album.getOauthURL(), 'POST').header("Authorization", "Basic " + Buffer.from(clientID + ':' + clientSecret).toString('base64')).body({ grant_type: 'client_credentials' }, 'form').send();
