...
export class albumMethods {
    ...
    async fetchByID(id: string): Promise<GetAlbumResp> {
        const fetchReq = await fetch(links.getAlbumURL(id)).header("Authorization", "Bearer " + this.accessToken).header("Content-Type", "application/json").send();
    ...
    async fetchSeveral(ids: string[]): Promise<GetAlbumResp[]> {
        const id = ids.join(',');
        const fetchReq = await fetch(links.getMultiAlbums(id)).header("Authorization", "Bearer " + this.accessToken).header("Content-Type", "application/json").send();
        const fetchRes = fetchReq.json();
    ...
    async fetchTracks(id: string): Promise<any> {
        const fetchReq = await fetch(links.getAlbumTracksURL(id)).header("Authorization", "Bearer " + this.accessToken).header("Content-Type", "application/json").send();
        const fetchRes = fetchReq.json();
